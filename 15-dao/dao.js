...
contract('DAO', (accounts) => {
  ...
  it('Should redeem share', async () => {
    ...
    assert(shares1.toNumber() === 0 );
    assert(availableFunds.toNumber() === 500);
  });

  it('Should NOT redeem share if not enough funds', async () => {
    await expectRevert(
      dao.redeemShare(100, {from: investor1}),
      'not enough shares'
    );
  });

  it('Should transfer share', async () => {
    await dao.transferShare(10, accounts[3], {from: investor2});
    const shares2 = await dao.shares(investor2);
    const shares3 = await dao.shares(investor3);
    const availableFunds = await dao.availableFunds();

    assert(shares2.toNumber() === 190 );
    assert(shares3.toNumber() === 310 );
    assert(availableFunds.toNumber() === 500);
  });

  it('Should NOT transfer share if not enough funds', async () => {
    await expectRevert(
      dao.transferShare(100, accounts[2], {from: investor1}),
      'not enough shares'
    );
  });
  ...
  it('Should execute proposal', async () => {
    await dao.createProposal('proposal 2', 400, accounts[8], {from: investor1});
  ...
  it('Should NOT redeem share if not enough funds', async () => {
    await expectRevert(
      dao.redeemShare(190, {from: investor2}),
