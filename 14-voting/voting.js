...
contract('Voting', (accounts) => {
  ...
  it('should NOT add voters if not admin', async () => {
    await expectRevert(
        voting.addVoters([voter1, voter2, voter3],{from: nonVoter}),
        'only admin'
      );
  });
  ...
  it('should vote only once for a Ballot', async () => {
    await voting.createBallot(
      'Ballot 4',
      ['Choice 1', 'Choice 2', 'Choice 3'],
      10,
  ...
  it('should vote', async () => {
    ...
    await voting.vote(4, 0, {from: voter1}),
    await voting.vote(4, 0, {from: voter2}),
    await voting.vote(4, 1, {from: voter3}),
    await time.increase(5001);
    const results = await voting.result(4); 
    assert(results[0].votes === '2');
    assert(results[1].votes === '1');
    assert(results[2].votes === '0');
  });

  it('should NOT dislay results before ballot end', async () => {
    await voting.createBallot(
        'Ballot 6',
        ['Choice 1', 'Choice 2', 'Choice 3'],
        10,
    ...
    await voting.vote(5, 0, {from: voter1}),
    await expectRevert(
        voting.result(5),
